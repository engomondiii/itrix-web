...
test('the forgot-password confirmation is IDENTICAL for a known and an unknown address', async ({ page }) => {
  ...
  for (const email of ['definitely-a-customer@example.com', 'nobody-here-at-all@example.invalid']) {
    ...
    seen.push((await page.locator('.auth-confirmation').innerText()).trim());
  }

  /* Word for word. Not "similar" — identical. */
  expect(seen[0]).toBe(seen[1]);
  expect(seen[0]).toContain('If that address has an itriX workspace');
});
...
test('the forgot-password page has no not-found state at all', async ({ page }) => {
  ...
  await page.route('**/api/auth/password-reset/request', (route) =>
    route.fulfill({ status: 404, contentType: 'application/json', body: JSON.stringify({ detail: 'no such client' }) }),
  );
  await page.goto('/forgot-password');
  ...
  await expect(page.locator('body')).not.toContainText('could not find');
  await expect(page.locator('body')).not.toContainText('not registered');
});
...
test('sign-in gives ONE message for a wrong password and an unknown address', async ({ page }) => {
  ...
  for (const scenario of [
    ...
    await expect(page.locator('.auth-error-summary')).toBeVisible();
    messages.push((await page.locator('.auth-error-summary li').first().innerText()).trim());
  }

  expect(messages[0]).toBe(messages[1]);
  await expect(page.locator('.auth-error-summary')).not.toContainText("don't recognise");
});
...
test('an empty field gives the same message as a wrong password', async ({ page }) => {
  ...
  await page.goto('/sign-in');
  await page.getByRole('button', { name: 'Sign in', exact: true }).click();
  await expect(page.locator('.auth-error-summary')).toContainText('Those details did not match');
});

test('an unknown, a used and an expired invitation code all give ONE message', async ({ page }) => {
  const messages: string[] = [];

  /* Three runs, one per cause. The loop body is identical on purpose: the proxy collapses
     all three into `{ usable: false }` with a 200, so there is nothing to vary. */
  for (let run = 0; run < 3; run += 1) {
    /* The proxy returns { usable: false } with a 200 for all three — not a 404 for one
       and a 410 for another, because the status code is as readable as the body. */
    await page.route('**/api/auth/invite/lookup**', (route) =>
      route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ usable: false }) }),
    );
    await page.goto('/sign-up');
    await page.getByLabel('Invitation code').fill('some-code-value-here');
  ...
  for (const word of ['expired code', 'already used', 'does not exist', 'unknown code']) {
...
test('the invite lookup returns no organisation, persona or email', async ({ page }) => {
  /* Everything this endpoint returns is a disclosure to an unauthenticated party. The
     proxy RE-SHAPES the backend answer, so a future backend field cannot leak through. */
  await page.route('**/api/auth/invite/lookup**', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
  ...
  const seen = await page.evaluate(async () => {
    const res = await fetch('/api/auth/invite/lookup?code=tok_abc');
    return (await res.json()) as Record<string, unknown>;
  }).catch(() => null);

  /* Run through the page context so the real proxy is exercised where available; when the
     route is mocked this still asserts the client never surfaces the extra fields. */
  if (seen) {
