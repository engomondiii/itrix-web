  ...
  return (
    <div
      className={cn(
        ...
        isClient ? 'border border-border-medium bg-surface' : 'border-l-[3px] border-ink-primary bg-soft',
      )}
    >
      <SenderLabel kind={message.senderKind} />
      <p className="mt-1 whitespace-pre-wrap text-body text-ink-primary">{message.body}</p>
    </div>
  );
}
