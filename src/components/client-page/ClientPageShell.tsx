...
export function ClientPageShell({ page }: { page: ClientPage }) {
  const relevanceTone: Record<'high' | 'medium' | 'low', string> = {
    ...
    <div className="container-page py-12">
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1.6fr_1fr] lg:items-start">
        ...
        <div className="flex flex-col gap-8">
              ...
              <ul className="mt-1 flex flex-col divide-y divide-line-subtle">
                {page.diagnosis.map((row) => (
                  <li key={row.label} className="flex items-center justify-between gap-4 py-2.5">
                    ...
                    <span className={cn('text-secondary font-medium capitalize', relevanceTone[row.relevance])}>
          ...
          {page.kpiPreview.length > 0 ? (
            <Card variant="default" className="flex flex-col gap-3">
              <SectionLabel>What an evaluation could measure</SectionLabel>
              <div className="grid gap-3 sm:grid-cols-3">
                {page.kpiPreview.map((kpi) => (
                  <div key={kpi.label} className="rounded-md border border-line-subtle bg-surface p-3">
          ...
          {page.proofPreview.length > 0 ? (
            <Card variant="default" className="flex flex-col gap-3">
              <SectionLabel>Proof</SectionLabel>
              <ul className="flex flex-col gap-2">
                {page.proofPreview.map((p) => (
