  ...
  return (
    <>
      ...
      <div className="mx-auto flex max-w-3xl flex-col gap-8 px-6 py-8">
        <header>
          <h1 className="font-display text-web-h2 text-ink-primary">{successCopy.support.title}</h1>
          <p className="mt-3 max-w-reading text-web-body text-ink-secondary">{successCopy.support.intro}</p>
        </header>

        <SupportComposer />

        <section aria-labelledby="open-requests" className="flex flex-col gap-3">
          <h2 id="open-requests" className="font-display text-web-h3 text-ink-primary">{ko ? '내 요청' : 'Your requests'}</h2>
          {loading ? (
