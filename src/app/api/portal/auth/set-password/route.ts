...
import { clearPasswordSetCapability, getPasswordSetCapability, setClientSession } from '@/lib/server/session';
...
export async function POST(req: Request) {
  ...
  const token = body.token || (await getPasswordSetCapability());
  ...
  const res = await djangoFetch<{ access?: string; refresh?: string; client?: ClientIdentity }>(
    ...
    { method: 'POST', authed: false, body: { token, password: body.password } },
  );
  if (!res.ok) {
    return NextResponse.json({ error: { detail: 'reset_failed' } }, { status: 400 });
  }
  if (res.data?.access) {
    await clearPasswordSetCapability();
