...
export const PORTAL_COPY_KO = {
  signIn: { title:'itriX 워크스페이스에 로그인', emailLabel:'이메일 주소', passwordLabel:'비밀번호', button:'로그인', forgot:'비밀번호를 잊으셨나요?', needAccess:'접근이 필요하신가요? itriX 전문가와 계속하기' },
  forgotPassword: { intro:'워크스페이스에서 사용하는 이메일 주소를 입력하면 비밀번호 재설정 링크를 보내드립니다.', button:'재설정 링크 보내기', confirmation:'해당 이메일이 등록되어 있는 경우 재설정 링크가 전송됩니다.' },
  setPassword: { title:'워크스페이스 비밀번호 설정', intro:'itriX 워크스페이스 설정을 마치려면 비밀번호를 정해 주세요.', passwordLabel:'새 비밀번호', confirmLabel:'비밀번호 확인', button:'비밀번호 설정 후 계속' },
  home: {
    welcome:(firstName:string)=>`${firstName}님, 다시 오신 것을 환영합니다.`,
    welcomeBody:'이곳은 비공개 itriX 워크스페이스입니다. 리뷰, itriX 팀과의 대화, 다음 단계를 모두 여기에서 이어갈 수 있습니다.',
  ...
  messages:{
    ...
    inbox:{ header:'메시지', listLabel:'대화', threadFallbackSubject:'itriX 팀과의 대화', empty:'아직 메시지가 없습니다. 아래에서 itriX 팀에 메시지를 보내면 이 목록에 대화가 표시됩니다.', briefingRow:'내 itriX 브리핑', briefingPreview:'itriX가 이해한 내용을 팀이 최신 상태로 유지하는 리뷰입니다.', briefingNotReady:'브리핑을 준비 중입니다. 준비되면 여기에 표시됩니다.', unreadLabel:(n:number)=>`읽지 않은 메시지 ${n}개`, teamJoinedTag:'itriX 팀' },
  },
  briefing:{ header:'내 itriX 리뷰', intro:'이 문서는 작업이 진행되면서 계속 업데이트됩니다. 워크로드를 더 이해하게 되면 여기에서 갱신하고 메시지로 알려드립니다.', lastUpdated:(date:string)=>`itriX 팀이 ${date}에 업데이트`, updateNotice:'최근 대화를 반영해 리뷰를 업데이트했습니다. 변경된 부분이 아래에 표시됩니다.' },
  documents:{
    header:'문서', intro:'itriX가 공유한 자료를 한곳에서 확인합니다. 제한 자료는 이 워크스페이스에 명시적으로 승인된 경우에만 표시됩니다. NDA는 공개를 보호할 수 있지만 그 자체로 접근 권한을 부여하지 않습니다.',
    openFolders:['내 리뷰','제품 개요','공개 기술 노트','평가 진행 방식'],
    dataRoomLocked:{ heading:'제한 자료 — 별도 승인 필요', body:'상세 기술 자료, 검증 결과, 근거 문서는 현재 작업 범위에서 해당 자료가 명시적으로 승인되고 필요한 계약 조건이 충족된 경우에만 공유됩니다.', button:'NDA 요청', bodyWithNda:'NDA가 체결되어 있습니다. 접근 권한은 여전히 명시적인 콘텐츠 승인과 작업 단계에 따라 별도로 결정됩니다. 이 상태만으로 제한 자료가 승인되지는 않습니다.' },
    ...
    confidentialityBanner:'적절한 보호가 마련되기 전에는 기밀 기술 정보를 제출하지 마세요. 제한 자료는 현재 작업 범위에서 명시적으로 승인되고 필요한 계약이 적용되는 경우에만 공유됩니다.',
  },
  evaluation:{ header:'내 평가', intro:'실제 워크로드를 대상으로 범위를 명확히 정한 유료 평가입니다. 각 단계를 여기에서 확인할 수 있고 산출물은 문서에 저장됩니다.', reportButton:'보고서 열기', measuresReminder:'워크로드에 따라 실행 시간, 메모리, 에너지, 정확도, 재현성, 통합 가능성을 측정할 수 있습니다. 이는 해당 사례에서 측정하는 값이며 사전에 보장되지 않습니다.', emptyState:'진행 중인 평가가 없습니다. itriX 팀과 평가에 합의하면 여기에서 진행 상황을 확인할 수 있습니다.' },
  poc:{ header:'내 개념검증(PoC)', intro:'실제 워크로드에서 itriX 접근법을 시험하는 별도의 단계입니다. 시작 전에 성공 기준과 범위를 명시적으로 합의합니다.', successNote:'PoC는 시작할 때 합의한 기준에 따라 판단합니다. 입증된 결과, 검증 중인 결과, 향후 가능성을 구분해 표시합니다.', emptyState:'진행 중인 PoC가 없습니다. 별도로 합의된 경우 이곳에 마일스톤이 표시됩니다.' },
  settings:{
    profileHeader:'내 프로필', profileFields:{fullName:'이름',email:'이메일 주소',organization:'조직',role:'역할',password:'비밀번호'}, saveProfile:'변경사항 저장',
    teamHeader:'내 팀', teamIntro:'동료를 이 워크스페이스에 초대해 함께 itriX를 평가할 수 있습니다. 초대된 사람은 같은 리뷰, 문서, 대화를 볼 수 있습니다.', invitePlaceholder:'colleague@company.com', sendInvite:'초대 보내기',
    notificationsHeader:'알림', notificationsIntro:'itriX가 이메일로 알려드릴 항목을 선택하세요.', notificationLabels:{newTeamMessage:'itriX 팀의 새 메시지',reviewUpdated:'리뷰 업데이트',evalOrPocStatus:'평가 또는 PoC 상태 변경',documentShared:'새 문서 공유'}, savePreferences:'알림 설정 저장', signOut:'로그아웃',
  },
  invite:{ accepting:'워크스페이스 설정 중…', welcomeTitle:'itriX 워크스페이스에 오신 것을 환영합니다', welcomeBody:'워크스페이스가 준비되었습니다. 리뷰, 대화, 다음 단계를 여기에서 이어갈 수 있습니다.', enterButton:'워크스페이스 들어가기', fallbackTitle:'곧 연락드리겠습니다', fallbackBody:'itriX 팀이 워크스페이스 개설 상태를 확인하고 있습니다. 그동안 리뷰는 계속 열어둘 수 있습니다.' },
} as const;
